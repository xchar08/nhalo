...
function ResearchChat({
  ...
  const [activeContext, setActiveContext] = useState<'latest' | 'root' | `branch:${string}`>('latest');
      ...
      <div className="w-full bg-[#050505] border border-white/10 rounded-xl shadow-2xl p-4 flex flex-col gap-4">
        <div className="flex items-center justify-between gap-2 text-cyan-400 text-xs font-mono uppercase border-b border-white/5 pb-2">
          ...
          {mode === 'analysis' && (
            <select
              value={activeContext}
              onChange={(e) => setActiveContext(e.target.value as any)}
              className="bg-black border border-white/10 text-gray-200 text-[10px] rounded px-2 py-1"
              title="Select chat context"
            >
              <option value="latest">Context: Latest</option>
              <option value="root">Context: Root</option>
              {branchIds.map((b) => (
        ...
        <form onSubmit={handleAsk} className="relative">
          ...
          />
          <button type="submit" disabled={loading} className="absolute right-2 top-2 p-1 text-cyan-500 hover:text-white disabled:opacity-50">
...
export default function ClientHome() {
  ...
  // NEW: session linkage for dive branches
  ...
  // NEW: branch ids list for chat selector
  const [branchIds, setBranchIds] = useState<string[]>([]);

  // Graph state (stateful so we can merge branch deltas)
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const result: any = await analyzePdrAction(inputText, deepMode, breadth);
      ...
      if (result.success) {
        ...
        // NEW: stash sessionId so dive branches can link to the same research session
        setSessionId(result.sessionId || null);

        // NEW: reset branch list on each new run
        setBranchIds([]);

        // Build initial graph (root branch)
        const rootBranch = 'root';
        const newNodes: GraphNode[] = [];
        const newLinks: GraphLink[] = [];

        (result.claims || []).forEach((c: Claim) => {
          const claimNodeId = `claim:${c.id}`;

          newNodes.push({
            id: claimNodeId,
            label: c.text.slice(0, 45) + (c.text.length > 45 ? '…' : ''),
          ...
          (c.evidence || []).forEach((ev) => {
            const docId = `doc:${c.id}:${ev.url}`;

            newNodes.push({
              id: docId,
              label: (ev.title || ev.url).slice(0, 40) + ((ev.title || ev.url).length > 40 ? '…' : ''),
        ...
        const uniq = Array.from(new Map(newNodes.map((n) => [n.id, n])).values());
        setGraphNodes(uniq);
  ...
  async function doDive() {
    ...
    try {
      ...
      const res: any = await diveDeeperAction({
        ...
        depth: diveDepth,
        breadth: diveBreadth,
        // NEW: link branch to current research session
      ...
      // NEW: add branch id so it appears in chat context dropdown
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 bg-white/5 p-1 rounded border border-white/10">
            <button
              ...
              className={`px-3 py-1 text-[10px] rounded ${mode === 'input' ? 'bg-cyan-500/20 text-cyan-400' : 'text-gray-400 hover:text-white'}`}
            >
              RESEARCH
            </button>
            <button
              onClick={loadFeed}
              className={`px-3 py-1 text-[10px] rounded ${mode === 'feed' ? 'bg-cyan-500/20 text-cyan-400' : 'text-gray-400 hover:text-white'}`}
            >
              KNOWLEDGE FEED
            </button>
            {claims.length > 0 && (
              <button
                ...
                className={`px-3 py-1 text-[10px] rounded ${mode === 'analysis' ? 'bg-cyan-500/20 text-cyan-400' : 'text-gray-400 hover:text-white'}`}
              >
                ANALYSIS
              </button>
            )}
          ...
          {mode === 'analysis' && (
            <div className="flex items-center gap-1 bg-black/40 p-1 rounded border border-white/10">
              <button
                ...
                className={`px-3 py-1 text-[10px] font-bold rounded transition-colors ${viewTab === 'graph' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'}`}
              >
                GRAPH
              </button>
              <button
                onClick={() => setViewTab('report')}
                className={`px-3 py-1 text-[10px] font-bold rounded transition-colors ${viewTab === 'report' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'}`}
              >
                REPORT
              </button>
              <button
                onClick={() => setViewTab('sources')}
                className={`px-3 py-1 text-[10px] font-bold rounded transition-colors ${viewTab === 'sources' ? 'bg-white text-black' : 'text-gray-400 hover:text-white'}`}
              >
                SOURCES
              </button>
            </div>
        ...
        <div className="flex items-center gap-4">
          {user ? (
            <div className="flex items-center gap-2 text-[10px] font-mono text-gray-400 bg-white/5 px-3 py-1 rounded-full border border-white/10">
              <User size={10} /> <span>{user.email}</span>
            </div>
          ) : (
            <button
          ...
          <div className="w-64 h-full border-r border-white/10 bg-black/40 backdrop-blur-sm hidden lg:flex flex-col z-40 relative print:hidden">
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
              {history.map((h) => (
                ...
                >
                  ...
                  <button onClick={(e) => handleDeleteItem(e, h.id)} className="opacity-0 group-hover:opacity-100 p-1 hover:text-red-400">
                    <Trash2 size={10} />
                  </button>
                </div>
              ))}
            ...
            {history.length > 0 && (
              <div className="p-3 border-t border-white/10">
                {!showWipeConfirm ? (
                  <button onClick={() => setShowWipeConfirm(true)} className="w-full flex items-center justify-center gap-2 text-[10px] text-gray-500 hover:text-red-400 py-2">
                    <Trash2 size={10} /> WIPE HISTORY
                  </button>
                ) : (
                  <div className="flex gap-2">
                    <button onClick={() => setShowWipeConfirm(false)} className="flex-1 bg-black/50 text-gray-400 text-[10px]">
                      NO
                    </button>
                    <button onClick={handleWipeHistory} className="flex-1 bg-red-900/40 text-red-200 text-[10px]">
                      YES
                    </button>
                  </div>
                )}
        ...
        <div className="flex-1 relative h-full overflow-hidden print:h-auto print:overflow-visible">
                ...
                <div className="relative bg-[#050505] rounded-xl border border-white/10 shadow-2xl overflow-hidden group">
                  <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-white/5">
                    <div className="flex items-center gap-2 text-[10px] font-mono text-gray-400">
                      <Command size={10} /> <span>SYSTEM_READY</span>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-4 justify-between items-center px-4 py-3 bg-white/5 border-t border-white/5">
                    <div className="flex items-center gap-4">
                      ...
                      >
                        <Layers size={12} /> {deepMode ? 'Deep: ON' : 'Deep: OFF'}
                      </button>
                ...
                <div className="grid gap-4">
                  {knowledgeFeed.map((item, i) => (
                    <div key={i} className="p-6 bg-white/5 rounded-lg border border-white/5 hover:border-cyan-500/30 transition-colors">
                      <a href={item.link} target="_blank" rel="noreferrer" className="text-lg font-bold text-gray-100 hover:text-cyan-400 transition-colors">
          ...
          {mode === 'analysis' && (
            <div className="absolute inset-0 flex w-full h-full bg-black print:bg-white print:relative print:block print:h-auto">
              ...
              {viewTab === 'graph' && (
                <>
                  <div className="w-[50%] h-full border-r border-white/10 bg-black/40 relative shrink-0 print:hidden">
                  ...
                  <div className="max-w-3xl w-full print:max-w-none">
                    <div className="flex justify-between items-center mb-8 border-b border-white/10 pb-4 print:border-black/20">
                      ...
                      <div className="flex gap-2 print:hidden">
                        <button onClick={() => handleExport('markdown')} className="flex items-center gap-2 px-3 py-1.5 bg-white/5 hover:bg-white/10 rounded text-xs text-gray-300">
                          <Download size={12} /> MD
                        </button>
                        <button onClick={() => handleExport('html')} className="flex items-center gap-2 px-3 py-1.5 bg-white/5 hover:bg-white/10 rounded text-xs text-gray-300">
                          <Download size={12} /> HTML
                        </button>
                        <button onClick={() => handleExport('pdf')} className="flex items-center gap-2 px-3 py-1.5 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 rounded text-xs">
                    ...
                    <div className="prose prose-invert prose-sm max-w-none print:prose-black">
                      <ReactMarkdown
                        ...
                        components={{
                          a: ({ node, ...props }) => (
                            <a {...props} className="text-cyan-400 hover:underline print:text-blue-600 print:no-underline" target="_blank" rel="noopener noreferrer" />
                          ),
                          code: ({ node, inline, className, children, ...props }: any) => {
                            return !inline ? (
                              ...
                              <code className="bg-white/10 px-1 py-0.5 rounded text-cyan-300 text-[0.9em] font-mono print:bg-gray-100 print:text-black" {...props}>
                                {children}
                              </code>
                            );
                          },
                          h1: ({ node, ...props }) => (
                            <h1 {...props} className="text-3xl font-bold text-white mb-4 border-b border-white/10 pb-2 print:text-black print:border-black" />
                          ),
                              ...
                              return (
                                <li className="list-none my-3">
                                  <div className="flex items-start gap-3 p-3 rounded-lg bg-cyan-950/30 border border-cyan-500/20 hover:border-cyan-500/40 transition-all print:bg-blue-50 print:border-blue-200">
                                    ...
                                    <div className="text-sm text-gray-200 print:text-gray-800 leading-relaxed">{children}</div>
                                  </div>
                                </li>
                              );
                            }
                          ...
                          blockquote: ({ node, ...props }) => (
                            <blockquote {...props} className="border-l-2 border-cyan-500 pl-4 italic text-gray-400 my-4 print:text-gray-700 print:border-blue-500" />
                          ),
                        }}
                      >
                        {unifiedReport}
                    ...
                    <div className="grid gap-4">
                      {rawSources.map((s, i) => (
                        <div key={i} className="p-4 bg-white/5 rounded border border-white/5 hover:border-cyan-500/30 transition-colors">
                          <a href={s.url} target="_blank" rel="noopener noreferrer" className="text-cyan-400 hover:underline text-sm font-bold break-all flex items-center gap-2">
                          ...
                          <div className="mt-2 flex gap-2">
                            <span className="text-[10px] bg-white/10 px-2 py-0.5 rounded text-gray-400">Score: {Math.round((s.score || 0) * 100)}%</span>
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center p-6 z-[100] print:hidden">
          <div className="w-full max-w-lg border border-white/10 bg-zinc-950 rounded p-4">
            ...
            <div className="flex justify-end gap-2 mt-4">
              <button onClick={() => setDiveModal({ open: false })} className="px-3 py-2 rounded border border-white/10 text-white/80 text-sm hover:bg-white/5">
                Cancel
              </button>
              <button
                disabled={busy}
